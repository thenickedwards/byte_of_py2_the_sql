...
SELECT product_name
    -- , units_in_stock, reorder_level, units_on_order 
    FROM products 
...
SELECT AVG(shipper_count) FROM shippers_per_customer; -- ~ 2.685
...
SELECT s.company_name, s.address, s.city, s.region, s.postal_code, s.country FROM suppliers s
	UNION
SELECT c.company_name, c.address, c.city, c.region, c.postal_code, c.country FROM customers c
	ORDER BY company_name; -- 120
...
-- in descending order.
SELECT company_name, SUM(od.quantity) FROM orders o
